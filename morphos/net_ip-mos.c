...
#	if !defined(__sun) && !defined(__sgi) && !defined(__MORPHOS__)
#		include <ifaddrs.h>
#	endif
...
{
	return "Error within getaddrinfo (it's all capehill's fault)";
}

int getaddrinfo(const char * nodename, const char * servname, const struct addrinfo * hints, struct addrinfo **res)
{
	//Com_Printf("getaddrinfo nodename: '%s'\n", nodename);
		...
		*res = malloc(sizeof(struct addrinfo));
		...
		if (*res)
		{
			...
			if (sin)
			{
				...
				memcpy(&(sin->sin_addr), he->h_addr, he->h_length + 1);
				...
				//Com_Printf("%s sin_addr: '%s'\n", __FUNCTION__, inet_ntoa( sin->sin_addr ) );
...
int getnameinfo(const struct sockaddr *sa, socklen_t salen, char *node, socklen_t nodelen, char * service, socklen_t servicelen, int flags)
{
	...
	//Com_Printf("getnameinfo: family %d passed\n", sa->sa_family);
	...
	strncpy(buf, inet_ntoa(sin->sin_addr), 31);
...
const char	*NET_AdrToString (netadr_t a)
{
	...
	else if (a.type == NA_IP /* || a.type == NA_IP6 */) // Cowcat
	{
		struct sockaddr_storage sadr;
...
qboolean	NET_CompareAdr (netadr_t a, netadr_t b)
{
	...
	if (a.type == NA_IP /* || a.type == NA_IP6 */ ) // Cowcat
	{
		if (a.port == b.port)
			return qtrue;
	}
...
qboolean NET_GetPacket(netadr_t *net_from, msg_t *net_message, fd_set *fdr)
{
	...
	{
		fromlen = sizeof(from);
		ret = recvfrom( ip_socket, (void *)net_message->data, net_message->maxsize, 0, (struct sockaddr *) &from, (long *)&fromlen ); // cast long* Cowcat
...
void Sys_SendPacket( int length, const void *data, netadr_t to ) {
	...
	if( to.type != NA_BROADCAST && to.type != NA_IP /* && to.type != NA_IP6 && to.type != NA_MULTICAST6 */ ) // Cowcat
	{
		//Com_Error( ERR_FATAL, "Sys_SendPacket: bad address type" );
		Com_Error( ERR_FATAL, "Sys_SendPacket: bad address type %d", to.type ); // test Cowcat
		return;
	}
...
SOCKET NET_IPSocket( char *net_interface, int port, int *err ) {
	...
	*err = 0;
	...
	if( ioctlsocket( newsocket, FIONBIO, (char *)&_true ) == SOCKET_ERROR ) { // cast char * Cowcat
...
#if defined (DEDICATED) || defined (__MORPHOS__)
